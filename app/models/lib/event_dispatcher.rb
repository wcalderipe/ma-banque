...
class Lib::EventDispatcher
  ...
  #   synchronously
  # * Reactors registered with `async` will be triggered
  ...
  def self.on(*events, trigger: [], async: [])
    # TODO: Need to implement a background job processor like Sidekiq
    #       to handle async reactors.
    rules.register(
  ...
  class RuleSet
    ...
    def for(event)
      ...
      @rules.each do |event_class, rule|
        # Match event by class including ancestors. e.g. All events
        # match a role for BaseEvent.
